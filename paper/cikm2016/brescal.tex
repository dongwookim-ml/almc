...
We propose a probabilistic framework that directly generalises \textsc{Rescal}
...
We develop a Gibbs sampler to perform the posterior inference for the probabilistic \textsc{Rescal} (\textsc{Brescal}). The conditional distribution of each latent variable is given by:
\begin{align}
...
$E$ and $R$ by Laplace approximation~\cite{bishop2006pattern}. The detail derivations are provided in the appendix\footnote{The appendix is avariable at: http://cm.cecs.anu.edu.au/prescal}.
\eat{
...
We show in the empirical experiments that \textsc{Brescal} outperforms standard \textsc{Rescal}.
