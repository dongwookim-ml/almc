...
\smallhead{Thompson Sampling} The probabilistic framework allows us to quantify the uncertainty of predictive distribution, which is then used to formulate an active learning algorithm. Specifically, we adopt Thompson sampling for the active learning, which finds an optimal trade off between exploitation and exploration during the active learning.
