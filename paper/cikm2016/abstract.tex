...
In this paper, we propose a new probabilistic knowledge base factorisation method that benefits from the path structure of existing knowledge (e.g. syllogism). This explicit probabilistic formulation enable us to develop an incremental population model based on exploitation-exploration strategies.
We demonstrate that the factorisation model with the path structure performs better on the knowledge completion task. Whereas the model without the path structure performs better in the incremental population.
