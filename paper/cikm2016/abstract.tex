...
In many cases, however, there is not enough external sources to extract information. An active knowledge population via labelling of human experts can help to reduce the gap between two processes. 
In this paper, we propose a new probabilistic knowledge base factorisation method that benefits from the graph path structure of existing knowledge (e.g. syllogism). This explicit probabilistic formulation enable us to develop an active population model based on exploitation-exploration strategies.
We demonstrate that the compositional knowledge factorisation results a better performance on the knowledge completion task. Whereas the model performs worse in the active population.
