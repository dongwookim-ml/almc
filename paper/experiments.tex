...
\begin{figure}[t]
	...
	\includegraphics[width=0.42\linewidth]{images/toy_logit_vs_normal_20_10_5.pdf}
	}
...
Adding one valid triple in a knowledge base will change the entire compositional path over the extended tensor $\mathcal{X}^L$.

The computational complexity will increase exponentially as we increase the length of the composition.

The results between active acquisition and training and testing are not always coincident, but still we can understand whether the assumption behind the compositionality holds. 

We set the compositional length $L$ to 2. We split each dataset into train/validation/test set.

We will leave the active acquisition of the compositional model a future work.
