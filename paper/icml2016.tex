...
Let $x_{(i, k_{\text{comp}(k_1, k_2)}, l)}$ bs a number of path from entity node $i$ to entity node $j$ through relation edges $k_1$ and $k_2$.

We model $x_{(i, k_{\text{comp}(k_1, k_2)}, l)} \sim \mathcal{N}(e_i^\top R_{\text{comp}(k_1,k_2)} e_j, \sigma_\text{comp}^2)$ where $R_{\text{comp}(k_1,k_2)} \in \mathbb{R}^{D\times D}$.
...
\begin{description}
  ...
  \begin{enumerate}
    ...
    \end{itemize}
  \end{enumerate}
  \item[Bayesian Tensor Factorisation] Some other related work
  \begin{itemize}
    \item \url{http://www.autonlab.org/autonweb/19394/version/4/part/5/data/225_Xiong.pdf}
    \item \url{http://www.shakirm.com/papers/ntfmcmc_paper.pdf}
    \item \url{http://www.sciencedirect.com/science/article/pii/S0165168411003537}
  \end{itemize}
\end{description}
