...
\textbf{Stiatistical relational learning}:
\begin{itemize}
\item RESCAL \cite{nickel2011three}
\item \cite{kemp2006learning}: Infinite relational model based on entity clustering.
\end{itemize}

\textbf{Bayesian tensor factorisation}:
\begin{itemize}
\item \cite{xiong2010temporal,schmidt2009probabilistic}:CANDECOMP/PARAFAC (CP) decomposition
\item \cite{yilmaz2012algorithms}: CP and TUCKER3
\end{itemize}

\textbf{Thompson Sampling}:
\begin{itemize}
\item \cite{kawale2015efficient}: Matrix factorisation recommendation
\item \cite{gopalan2014thompson}: Thompson Sampling for Complex Online Problems
\item \cite{kocak2014spectral}: Spectral Thompson sampling
\end{itemize}

\textbf{Knowledge Graph}:
\begin{itemize}
\item \cite{kajino2015active}: Active learning for KG (populating KG)
\end{itemize}
\section{Model}
...
$(i, k_{\text{comp}(k_1, k_2)}, l)$

This compositional triple may have multiple path over the knowledge graph.
Both pairs of triples $(i, k_1 ,j)$,  $(j, k_2, l)$ and $(i, k_1 ,m)$,  $(m, k_2, l)$ can be represented as $(i, k_{\text{comp}(k_1, k_2)}, l)$.

Let $x_{(i, k_{\text{comp}(k_1, k_2)}, l)}$ be the number of path from entity $i$ to entity $j$ through relation edges $k_1$ and $k_2$.

We model the degree of the compositional relation as
\begin{align}
x_{(i, k_{\text{comp}(k_1, k_2)}, l)} \sim \mathcal{N}(e_i^\top R_{\text{comp}(k_1,k_2)} e_j, \sigma_\text{comp}^2),
\end{align}
where $R_{\text{comp}(k_1,k_2)} \in \mathbb{R}^{D\times D}$.
